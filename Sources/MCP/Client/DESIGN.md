...
| Recognized modern protocol error | Pin modern and surface the typed modern error | continue on the same connection |
...
Client-focused tests own explicit modern/legacy negotiation, same-connection
stdio fallback, malformed/unsupported responses, pending cleanup, per-request
